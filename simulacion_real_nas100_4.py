...
# -*- coding: utf-8 -*-
"""
Simulación de Trading Real - NAS100 (Índices) con Resiliencia
Terminal ID: 4
Proxy: BTCUSDT (Bitcoin como proxy para índices)
"""
...
class ResilientNAS100Simulator:
    def __init__(self):
        self.max_retries = 5
    ...
    def log_error(self, error_msg, exception=None):
        ...
        try:
            with open('logs_simulacion/NAS100_errors.jsonl', 'a', encoding='utf-8') as f:
    ...
    def create_simulator(self):
        ...
        simulator = BinanceRealDataSimulator(
            symbol="BTCUSDT",  # Bitcoin como proxy para NAS100
            initial_capital=1000.0,
            terminal_id=4
        )
        
        # Configurar parámetros específicos para índices
        simulator.win_rate = 0.68  # 68% win rate para índices
        simulator.avg_return = 0.012  # 1.2% retorno promedio
        simulator.volatility = 0.025  # 2.5% volatilidad
        simulator.trade_frequency = 300  # Trade cada 5 minutos
    ...
    def run_with_resilience(self):
        ...
        while self.running and self.restart_count < self.max_restarts:
            try:
                print(f"\n🎯 Iniciando Simulación NAS100 (Terminal 4) - Intento #{self.restart_count + 1}")
                print(f"📊 Proxy: BTCUSDT")
                ...
                simulator.log_file = f"logs_simulacion/NAS100_{timestamp}.jsonl"
                simulator.report_file = f"reportes_simulacion/NAS100_{timestamp}.json"
                
                # Ejecutar simulación con reintentos
                for attempt in range(self.max_retries):
                    try:
                        simulator.run_simulation()
                        print("✅ Simulación NAS100 completada exitosamente")
                ...
                error_msg = f"Fallo crítico en simulación NAS100 (restart #{self.restart_count})"
        ...
        print("🏁 Simulación NAS100 finalizada")

def main():
    """Función principal con resiliencia"""
    resilient_sim = ResilientNAS100Simulator()

...
from config import settings
from utils.message_queue import mq
from utils.order_executor import evaluar_y_ejecutar_operacion, monitor_open_positions_for_sl_tp
from utils.risk_manager import perform_pre_execution_risk_checks
from utils.state_manager import StateManager
from utils.structured_logger import setup_structured_logger
import uuid
...
async def main() -> None:
    ...
    last_sl_tp_check_time = time.time() # Initialize the last check time
    ...
    while True:
        ...
        # Check for SL/TP periodically
        current_time = time.time()
        if current_time - last_sl_tp_check_time >= settings.SL_TP_MONITOR_INTERVAL_SECONDS:
            logger.info("Ejecutando monitoreo de SL/TP.")
            await monitor_open_positions_for_sl_tp(bot_instance=None, chat_id=None)
            last_sl_tp_check_time = current_time

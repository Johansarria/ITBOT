#!/usr/bin/env python3
"""
Estrategia de Probabilidad Dinámica - ITBOT (Versión Sintética)
Basada en algoritmo validado con 64.99% retorno mensual
Implementa señales tipo A, B, C, D con filtros de calidad
Usa datos sintéticos para evitar dependencias externas
"""
...
class DynamicProbabilityStrategy:
    ...
    def generate_synthetic_data(self, symbol: str, days: int = 30) -> pd.DataFrame:
        ...
        returns = np.random.normal(0.0002, 0.015, hours)  # Retornos horarios
        prices = [base_price]
        
        for i in range(1, hours):
            # Añadir algo de momentum y reversión a la media
            momentum = 0.1 * returns[i-1] if i > 0 else 0
            mean_reversion = -0.05 * (prices[-1] / base_price - 1)
        ...
        for i in range(hours):
            vol_multiplier = 1 + abs(returns[i]) * 10  # Mayor volumen con mayor volatilidad
        ...
        df = pd.DataFrame({
            ...
            'high': [p * np.random.uniform(1.0, 1.02) for p in prices[:data_length]],
            'low': [p * np.random.uniform(0.98, 1.0) for p in prices[:data_length]],
    ...
    def calculate_signal_probability(self, data: pd.DataFrame, index: int) -> Tuple[float, str]:
        """Calcula probabilidad de señal y tipo"""
        if index < 20:  # Necesitamos historial mínimo
            return 0.0, 'none'
        ...
        elif current_price <= data.iloc[max(0, index-10):index]['close'].min() * 1.002:
    ...
    def check_quality_filters(self, data: pd.DataFrame, index: int) -> bool:
        """Verifica filtros de calidad de señal"""
        if index < 20:
            return False
    ...
    def open_position(self, symbol: str, entry_price: float, tp_pct: float, sl_pct: float, signal_type: str):
        """Abre nueva posición"""
        # Usar capital inicial para evitar compounding exponencial irreal
    ...
    def check_exit_conditions(self, data: pd.DataFrame, index: int):
        ...
        # Remover posiciones cerradas (en orden inverso)
    ...
    def close_position(self, position_index: int, exit_price: float, exit_reason: str, pnl_pct: float):
        ...
        # Calcular PnL correctamente: solo sobre el monto invertido
        pnl_amount = position['position_size'] * pnl_pct
        # El capital se actualiza con el PnL, no se suma el position_size completo
    ...
    def should_stop_trading(self) -> bool:
        """Verifica si debe parar el trading por gestión de riesgo"""
    ...
    def run_backtest(self, symbols: List[str] = None, days: int = 30) -> Dict:
        ...
        print("🎲 INICIANDO BACKTEST - Estrategia de Probabilidad Dinámica")
        ...
        for symbol in symbols:
            all_data[symbol] = self.generate_synthetic_data(symbol, days)
        
        # Simular trading hora por hora
        total_hours = days * 24
        
        for hour in range(20, total_hours):  # Empezar después de tener historial
        ...
        self.positions = []  # Limpiar posiciones
    ...
    def generate_results(self) -> Dict:
        ...
        # Calcular retorno mensual estimado
        monthly_return = total_return_pct  # Ya es para 30 días
    ...
    def print_results(self, results: Dict):
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            f.write("RESULTADOS - Estrategia de Probabilidad Dinámica\n")
            f.write("="*50 + "\n\n")
...
def main():
    """Función principal"""
    print("🎲 ESTRATEGIA DE PROBABILIDAD DINÁMICA - ITBOT")
    print("Replicando algoritmo validado con 64.99% retorno mensual")
    print("="*60)
